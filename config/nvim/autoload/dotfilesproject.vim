...
"
" Settings:
" b:dotfilesproject#roots [array] - look for config files in this array of
"                              directory names relative to the project
"                              root if it is set
" g:dotfilesproject#roots [array] - global overrides
" ============================================================================
...
function! dotfilesproject#GetRoot(...) abort
  if exists('b:dotfilesproject_root')
    return b:dotfilesproject_root
  endif

  let l:path = dotfilesproject#GetRootPath(a:000)
  let l:root = dotfilesproject#GetGitRootByFile(l:path)
  let l:root = empty(l:root)
        \ ? dotfilesproject#GetRootByFileMarker(s:default_markers)
        \ : l:root
  let l:root = empty(l:root)
        \ ? expand('%:p:h')
        \ : l:root

  let b:dotfilesproject_root = l:root
  return l:root
endfunction

" @param {String} [a:1] optional file for which you want the project root
" @return {String} path to project root
function! dotfilesproject#GetRootPath(...) abort
  " Argument
  " Path for given file
...
function! dotfilesproject#GetGitRootByFile(path) abort
  let l:std = split(
...
function! dotfilesproject#GetRootByFileMarker(markers) abort
  let l:result = ''
...
" paths relative to the root from dotfilesproject#GetRoot
"
" @return {String[]} config paths relative to dotfilesproject#GetRoot
function! dotfilesproject#GetPaths() abort
  return get(
        \   b:, 'dotfilesproject#roots', get(
        \   g:, 'dotfilesproject#roots',
        \   s:default_roots
        \ ))
endfunction

" Get full path to a dir in a dotfilesproject
"
" @param {String} dirname
" @return {String} full path to dir
function! dotfilesproject#GetDir(dirname) abort
  if empty(dotfilesproject#GetRoot())
    return ''
  endif

  for l:root in dotfilesproject#GetPaths()
    let l:current =
          \ expand(dotfilesproject#GetRoot() . '/' . l:root)
...
" Get full path to a file in a dotfilesproject
"
" @param {String} filename
" @return {String} full path to config file
function! dotfilesproject#GetFile(filename) abort
  " Try to use nearest first; findfile .; goes from current file upwards
...
" @return {String}
function! dotfilesproject#GetBin(bin) abort
  if empty(a:bin)
    return ''
  endif

  " Use cached
  let l:bins = dotfiles#InitDict('b:dotfilesproject_bin')
  if !empty(get(l:bins, a:bin))
    return l:bins[a:bin]
  endif

  let l:exe = dotfilesproject#GetFile(a:bin)
...
function! dotfilesproject#GetEslintrc() abort
  let l:candidates = map(copy(s:eslintrc_candidates), 'dotfilesproject#GetFile(v:val)')
  return call('dotfiles#First', l:candidates)
endfunction
