if exists('g:loaded_dotfiles') | finish | endif
let g:loaded_dotfiles = 1

let g:dotfiles#vim_dir = fnamemodify(resolve(expand('$MYVIMRC')), ':p:h')
function! dotfiles#Runtimepath() abort
  put! =split(&runtimepath, ',', 0)
endfunction

" Declare and define var as new dict if the variable as not been used before
"
" @param {String} var
" @return {String} the declared var name
function! dotfiles#InitDict(var) abort
...
" @param  {String} var
" @return {String} the declared var name
function! dotfiles#InitList(var) abort
  let {a:var} = exists(a:var) ? {a:var} : []
  return {a:var}
endfunction
...
function! dotfiles#MapAll(settings) abort
...
function! dotfiles#Unique(list) abort
...
function! dotfiles#GetGrepper() abort
...
function! dotfiles#IsPlugged(name) abort
  ...
  let l:is_plugged = exists("g:plugs['" . a:name . "']")
        ...
        \      || isdirectory(expand(g:dotfiles#vim_dir . '/mine/' . a:name)) )
...
function! dotfiles#IsMakerExecutable(name, ...) abort
...
function! dotfiles#ShortPaths(pathlist, ...) abort
...
function! dotfiles#GetMru() abort
  " Shortened(Readable(Whitelist)
  return dotfiles#ShortPaths(filter(copy(v:oldfiles), s:mru_blacklist))
endfunction

" @return {List} listed buffers
function! dotfiles#GetBuffers() abort
  return map(
