...
---  * self
function m:start()
	m.watcher = hs.application.watcher
...
m.switch = function(space)
	if space ~= nil then
		...
		if m.parsedQuery.duration then -- make this a timed session
			m.timer = hs.timer.doAfter(m.parsedQuery.duration * 60, function()
				hs.sound.getByName('Blow'):play()
				hs.alert('⏲: Time is up!', moduleStyle)
				m.choose()
				m.timer = nil
			end)
		end
...
m.timerStr = function(runningTimer)
	...
	if runningTimer and runningTimer.data then
		...
		if m.timer then
			duration = ''
				.. math.ceil(m.timer:nextTrigger() / 60)
				.. 'm remaining'
		else
			duration = math.floor(
				(hs.timer.secondsSinceEpoch() + runningTimer.data.duration) / 60
			) .. 'm'
		end
