...
if not meg then return end
if not vim.g.enabled_plugin["megline"] then
...
local H = require("meg.utils.highlights")
...
local function get_diagnostics(seg_formatters_status)
  ...
  local diags = {
    { num = count(vim.diagnostic.severity.ERROR), sign = meg.icons.lsp.error, hl = "StError" },
    { num = count(vim.diagnostic.severity.WARN), sign = meg.icons.lsp.warn, hl = "StWarn" },
    { num = count(vim.diagnostic.severity.INFO), sign = meg.icons.lsp.info, hl = "StInfo" },
    { num = count(vim.diagnostic.severity.HINT), sign = meg.icons.lsp.hint, hl = "StHint" },
  }
...
local function get_hydra_status()
  local ok, hydra = meg.require("hydra.statusline")
  ...
  return seg(fmt("%s %s", meg.icons.misc.hydra, string.upper(data.name)), data.color)
end

local function get_substitution_status()
  local ok, cool = meg.require("cool-substitute.status")
  if not ok then return "" end
...
local function parse_filename(truncate_at)
  ...
  if not fname or meg.empty(fname) then return "", "", "No Name" end
...
local function seg_filename(truncate_at)
  ...
  -- usually our custom titles, like for megterm, neo-tree, etc
...
local function seg_prefix(truncate_at)
  ...
  local prefix = is_truncated(truncate_at) and "" or meg.icons.misc.lblock
  return seg(prefix, mode_info.hl)
end

local function seg_suffix(truncate_at)
  local mode_info = MODES[api.nvim_get_mode().mode]
  local prefix = is_truncated(truncate_at) and "" or meg.icons.misc.rblock
  return seg(prefix, mode_info.hl)
end
...
local function seg_lsp_status(truncate_at)
  ...
  if vim.tbl_isempty(messages) then
    local ok_nls, nls = meg.require("null-ls")
    local enabled = ok_nls and nls.enabled

    return get_diagnostics(seg(meg.icons.lsp.kind.Null, "StModeInsert", enabled))
  end
...
local function seg_git_symbol(truncate_at)
  ...
  local symbol = is_truncated(truncate_at) and "" or meg.icons.git.symbol
...
local function seg_git_status(truncate_at)
  ...
  local branch = is_truncated(truncate_at) and meg.truncate(status.head or "", 15, false) or status.head
...
function _G.__statusline()
  ...
  if is_plain() then
    local parts = {
      seg([[%<]]),
      seg_filename(),
      seg(fmt("[%s]", meg.icons.modified), "StModified", M.ctx.modified, { margin = { 0, 1 } }), -- alts: "%m"
      seg(meg.icons.misc.lock, "StModified", M.ctx.readonly, { margin = { 0, 1 } }), -- alts: "%r"
      "%=",
    }
