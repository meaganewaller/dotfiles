if not mw or not mw.mappings.enable then return end
...
-- Terminal {{{
------------------------------------------------------------------------------//
mw.augroup("AddTerminalMappings", {
  event = { "TermOpen" },
  pattern = { "term://*" },
  command = function()
...
if mw.falsy(fn.mapcheck("<ScrollWheelDown>")) then nmap("<ScrollWheelDown>", "<c-d>") end
if mw.falsy(fn.mapcheck("<ScrollWheelUp>")) then nmap("<ScrollWheelUp>", "<c-u>") end
------------------------------------------------------------------------------
-- Buffers
------------------------------------------------------------------------------
...
nnoremap("<leader><localleader>", [[<c-^>]], { desc = "switch to last buffer" })
-----------------------------------------------------------------------------//
-- Capitalize
-----------------------------------------------------------------------------//
...
nnoremap("<leader>ep", fmt("<Cmd>edit %s/lua/mw/plugins/init.lua<CR>", fn.stdpath("config")), {
...
function mw.mappings.setup_map() nnoremap("M", [[:nnoremap M n@z<CR>q:<C-u>let @z=strpart(@z,0,strlen(@z)-1)<CR>n@z]]) end

vim.g.mc = mw.replace_termcodes([[y/\V<C-r>=escape(@", '/')<CR><CR>]])
xnoremap("cn", [[g:mc . "``cgn"]], { expr = true, silent = true })
xnoremap("cN", [[g:mc . "``cgN"]], { expr = true, silent = true })
nnoremap("cq", [[:\<C-u>call v:lua.mw.mappings.setup_map()<CR>*``qz]])
nnoremap("cQ", [[:\<C-u>call v:lua.mw.mappings.setup_map()<CR>#``qz]])
xnoremap("cq", [[":\<C-u>call v:lua.mw.mappings.setup_map()<CR>gv" . g:mc . "``qz"]], { expr = true })
xnoremap(
  "cQ",
  [[":\<C-u>call v:lua.mw.mappings.setup_map()<CR>gv" . substitute(g:mc, '/', '?', 'g') . "``qz"]],
  { expr = true }
)
----------------------------------------------------------------------------------
-- Grep Operator
...
---@param type string
---@return nil
function mw.mappings.grep_operator(type)
...
nnoremap("<leader>g", function()
  vim.o.operatorfunc = "v:lua.mw.mappings.grep_operator"
  return "g@"
end, { expr = true, desc = "grep operator" })
xnoremap("<leader>g", ":call v:lua.mw.mappings.grep_operator(visualmode())<CR>")
-----------------------------------------------------------------------------//
...
-----------------------------------------------------------------------------//
nnoremap("<leader>ls", mw.list.qf.toggle, { desc = "toggle quickfix list" })
nnoremap("<leader>ll", mw.list.loc.toggle, { desc = "toggle location list" })
-----------------------------------------------------------------------------//
-- Completion
-----------------------------------------------------------------------------//
