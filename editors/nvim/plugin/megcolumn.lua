...
if not vim.g.enabled_plugin["megcolumn"] then return end
--
local fn, v, api = vim.fn, vim.v, vim.api
local ui, separators = meg.ui, meg.icons.separators
...
local function click(name, item) return "%@v:lua.meg.ui.statuscolumn." .. name .. "@" .. item end
...
--   local num = vim.wo[win].relativenumber and not meg.falsy(v.relnum) and v.relnum or v.lnum
...
local function nr(win, _line_count)
  if v.virtnum < 0 then return shade end -- virtual line
  if v.virtnum > 0 then return space end -- wrapped line
  local num = vim.wo[win].relativenumber and not meg.empty(v.relnum) and v.relnum or v.lnum
  local lnum = fn.substitute(num, "\\d\\zs\\ze\\" .. "%(\\d\\d\\d\\)\\+$", ",", "g")
  local num_width = (vim.wo[win].numberwidth - 1) - api.nvim_strwidth(lnum)
...
local function sep(is_active)
  local separator_hl = ""
  if is_active then
    separator_hl = v.virtnum >= 0 and meg.empty(v.relnum) and active_border_hl or ""
  else
...
function ui.statuscolumn.render(is_active)
  ...
  -- local is_absolute_lnum = v.virtnum >= 0 and meg.falsy(v.relnum)
...
meg.augroup("MegColumn", {
  {
    ...
    command = function(args)
      ...
      if buf.bt ~= "" or vim.tbl_contains(excluded, buf.ft) then
        ...
        vim.opt_local.statuscolumn = [[%!v:lua.meg.ui.statuscolumn.render(v:true)]]
      end
    end,
  },
  {
    ...
    command = function(args)
      ...
      if buf.bt ~= "" or vim.tbl_contains(excluded, buf.ft) then
        ...
        vim.opt_local.statuscolumn = [[%!v:lua.meg.ui.statuscolumn.render(v:false)]]
      end
    end,
  },
})
