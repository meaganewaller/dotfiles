...
mw.augroup("Utilities", {
  ...
  nested = true,
  command = function()
    if mw.falsy(vim.bo.filetype) or fn.exists("b:ftdetect") == 1 then
      cmd([[
        unlet! b:ftdetect
        filetype detect
...
mw.augroup("TerminalAutocommands", {
  ...
  command = function(args)
    ...
    if mw.falsy(v.event.status) and mw.falsy(vim.bo[args.buf].ft) then cmd.bdelete({ args.buf, bang = true }) end
  end,
})
