  ...
  keys = {
    {
      ...
      function()
        ...
        if is_floating_win(0) then vim.api.nvim_win_close(0, false) end
...
local function is_open()
  ...
  for _, v in pairs(dir_to_buf) do
    if bufnr == v then return true end
  end
  return false
end
      ...
      callback = function()
        vim.defer_fn(function()
          for _, winid in ipairs(vim.api.nvim_list_wins()) do
            if vim.api.nvim_win_get_buf(winid) == bufnr then vim.api.nvim_win_close(winid, true) end
          end
        end, 10)
      end,
    })
    ...
    vim.fn.termopen(vim.o.shell, {
      on_exit = function(j, c)
        for _, win in ipairs(vim.api.nvim_list_wins()) do
          if vim.api.nvim_win_get_buf(win) == bufnr then vim.api.nvim_win_close(win, true) end
        end
