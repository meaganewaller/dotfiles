...
local C = require("meg.lush_theme.colors")
local M = {
  blink_delay = 150,
  minimal_jump = 20,
...
function meg.blink_cursorline(delay)
  if is_ignored() then return end
  -- local tint_ok, tint = meg.require("tint")
...
local function enable_cursorline(should_blink)
  ...
  if should_blink then meg.blink_cursorline() end
