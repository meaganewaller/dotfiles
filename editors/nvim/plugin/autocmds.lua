      ...
      if not vim.g.started_by_firenvim then
        if vim.fn.argc() > 1 then
          vim.schedule(function()
            meg.resize_windows(args.buf)
            require("virt-column").refresh()
          end, 0)
          -- elseif vim.fn.argc() == 0 then
          --   require("meg.start").start()
        end
      end
    end,
  },
...
do
  ...
  --   return meg.empty(fn.win_gettype())
  --     and meg.empty(vim.bo.buftype)
  --     and not meg.empty(vim.bo.filetype)
...
do
  ...
  local function stop_hl()
    ...
    api.nvim_feedkeys(meg.replace_termcodes("<Plug>(StopHL)"), "m", false)
  end
  ...
  augroup("IncSearchHighlight", {
    ...
    },
    {
      ...
      command = function(evt)
        if vim.bo[evt.buf].filetype == "megterm" then return end
        stop_hl()
      end,
    },
    {
