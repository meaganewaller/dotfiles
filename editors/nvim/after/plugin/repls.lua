...
meg.command("TermElixir", function(args)
  -- local pre_cmd = ""
  local cmd = "iex"
  -- load up our Deskfile if we have one..
  -- if require("meg.utils").root_has_file("Deskfile") then pre_cmd = "eval $(desk load)" end

  if args.bang then
    cmd = fmt("elixir %s", vim.fn.expand("%"))
  elseif require("meg.utils").root_has_file("mix.exs") then
    cmd = "iex -S mix"
  end
...
meg.command("TermRuby", function(args)
  -- local pre_cmd = ""
  local cmd = ""
  -- if require("meg.utils").root_has_file("Deskfile") then pre_cmd = "eval $(desk load)" end
  ...
  elseif require("meg.utils").root_has_file("Gemfile") then
    cmd = "rails c"
  else
    cmd = "irb"
  end
...
meg.command("TermPython", function()
  local cmd = "python"
...
meg.command("TermNode", function(args)
  local cmd = "node"
