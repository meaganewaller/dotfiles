...
-- @usage local heirline_component = { init = require("utils.status").init.breadcrumbs { padding = { left = 1 } } }
    ...
    if opts.max_depth and opts.max_depth > 0 then
      ...
      if start_idx > 0 then
        table.insert(
          children,
          {
            provider = require("utils").get_icon "Ellipsis"
                .. opts.separator,
          }
        )
      end
...
---@return function # The Heirline init function.
-- @usage local heirline_component = { init = require("utils.status").init.separated_path { padding = { left = 1 } } }
...
---@return function # The Heirline init function.
-- @usage local heirline_component = { init = require("utils.status").init.update_events { "BufEnter", { "User", pattern = "LspProgressUpdate" } } }
