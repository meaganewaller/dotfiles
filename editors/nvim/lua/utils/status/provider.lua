...
-- @usage local heirline_component = { provider = require("utils.status").provider.fill }
...
-- @usage local heirline_component = { provider = require("utils.status").provider.signcolumn }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.numbercolumn }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.foldcolumn }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.tabnr() }
function M.tabnr()
...
-- @usage local heirline_component = { provider = require("utils.status").provider.spell() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.paste() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.macro_recording() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.showcmd() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.search_count() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.mode_text() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.percentage() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.ruler({ pad_ruler = { line = 3, char = 2 } }) }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.scrollbar() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.close_button() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.filetype() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.filename() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.file_encoding() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.file_format() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.unique_path() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.file_modified() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.file_read_only() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.file_icon() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.git_branch() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.git_diff({ type = "added" }) }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.diagnostics({ severity = "ERROR" }) }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.lsp_progress() }
-- @see utils.status.utils.stylize
function M.lsp_progress(opts)
  local spinner = utils.get_spinner("LSPLoading", 1) or { "" }
  return function()
    local _, Lsp = next(BrioVim.lsp.progress)
    return status_utils.stylize(
      Lsp
      and (
...
-- @usage local heirline_component = { provider = require("utils.status").provider.lsp_client_names({ expand_null_ls = true, truncate = 0.25 }) }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.treesitter_status() }
-- @see utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("utils.status").provider.str({ str = "Hello" }) }
-- @see utils.status.utils.stylize
function M.str(opts)
...
-- @usage local heirline_component = { provider = require("utils.status").provider.compiler_state() }
-- @see utils.status.utils.stylize
