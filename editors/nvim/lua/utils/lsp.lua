...
BrioVim.lsp = { progress = {} }             -- globally accesible
...
M.on_attach = function(client, bufnr)
  ...
  for id, _ in pairs(BrioVim.lsp.progress) do -- clear lingering progress messages
    if not next(vim.lsp.get_active_clients { id = tonumber(id:match "^%d+") }) then BrioVim.lsp.progress[id] = nil end
  end
  ...
  if server_name == "lua_ls" then -- by default initialize neodev and disable third party checking
    ...
    lsp_opts.before_init = function(param, config)
      if vim.b.neodev_enabled then
        for _, briovim_config in ipairs(BrioVim.supported_configs) do
          if param.rootPath:match(briovim_config) then
            table.insert(config.settings.Lua.workspace.library, BrioVim.install.home .. "/lua")
            break
          end
        end
      end
