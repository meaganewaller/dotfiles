...
---                                BrioVimBufsUpdated autocmd event.
...
function M.sort(compare_func, skip_autocmd)
  ...
  if type(compare_func) == "function" then
    ...
    vim.t.bufs = bufs
    if not skip_autocmd then require("utils").event "BufsUpdated" end
...
function M.close_tab()
  if #vim.api.nvim_list_tabpages() > 1 then
    vim.t.bufs = nil
    require("utils").event "BufsUpdated" -- Emit BrioVimBufsUpdated event
    vim.cmd.tabclose()
  end
end
