...
function M.generate_snapshot(write)
  local file
  local prev_snapshot = require(BrioVim.updater.snapshot.module)
  ...
  if write == true then
    file = assert(io.open(BrioVim.updater.snapshot.path, "w"))
    file:write "return {\n"
  end
...
function M.version(quiet)
  ...
  if BrioVim.updater.options.channel ~= "stable" then
...
function M.create_rollback(write)
  ...
  if write == true then
    local file = assert(io.open(BrioVim.updater.rollback_file, "w"))
    file:write(
...
function M.rollback()
  ...
  pcall(dofile, BrioVim.updater.rollback_file)
...
function M.update(opts)
  if not opts then opts = BrioVim.updater.options end
  opts = require("utils").extend_tbl(
  ...
  if is_stable then               -- if stable get tag commit
    local version_search = BrioVim.updater.stable_version_release or "latest"
  ...
  then
    ...
    if
      ...
      and not confirm_prompt {
        {
          "Unable to pull due to local modifications to BrioVim files.\nReset local files and continue?",
          "Error",
        },
        { "Reset local files and continue?" },
      }
    then
      echo(cancelled_message)
      return
      -- if continued and there were errors reset the BrioVim config and attempt another update
