...
M.run_action = function(name, fallback)
  name = name:lower()
  local actions = vim.tbl_filter(
    function(action) return not action.condition or action.condition() end,
    registered_actions[name]
  )

  if #actions == 0 then
    if fallback then vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes(fallback, true, true, true), "n", false) end
    return
...
M.add = function(name, action)
  name = name:lower()
  if not registered_actions[name] then registered_actions[name] = {} end
