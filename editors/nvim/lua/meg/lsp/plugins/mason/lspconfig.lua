...
local function on_attach(client, bufnr)
	require("meg.lsp").on_attach(client, bufnr)
	require("meg.lsp.plugins.lspsaga").on_attach(client, bufnr)
end
...
mason_lspconfig.setup_handlers({
	function(server)
		...
		local server_settings_ok, server_settings = pcall(require, "meg.lsp.settings." .. server)
		if server_settings_ok then
			opts = vim.tbl_deep_extend("keep", server_settings, opts)
		end
		...
		if server == "jsonls" or "tsserver" then
