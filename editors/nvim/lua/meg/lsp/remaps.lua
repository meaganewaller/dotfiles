...
local function generate_buf_keymapper(bufnr)
  return function(type, input, output, description, extraOptions)
    ...
    utils.noremap(type, input, output, description, options)
  end
end
...
function M.set_default_on_buffer(client, bufnr)
  ...
  buf_set_keymaps("n", "<Leader>rf", function()
  ...
  if ft == "sh" or ft == "lua" then
    buf_set_keymap("n", "<Leader>ld", function()
      ...
      local msgs = vim.diagnostic.get(buf_set_keymap)
