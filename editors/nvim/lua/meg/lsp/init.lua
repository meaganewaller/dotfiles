...
local function show_related_locations(diag)
  ...
  for _, info in ipairs(related_info) do
    diag.message = ("%s\n%s(%d:%d)%s"):format(
      ...
      not meg.empty(info.message) and (": %s"):format(info.message) or ""
    )
  end
  return diag
end
...
local function setup_commands(bufnr)
  ...
  local function make_diagnostic_qf_updater()
    ...
    return function()
      ...
      if not meg.is_vim_list_open() and cmd_id then
      ...
      cmd_id = api.nvim_create_autocmd("DiagnosticChanged", {
        callback = function()
          if meg.is_vim_list_open() then
            pcall(vim.diagnostic.setqflist, { open = false })
            if #fn.getqflist() == 0 then meg.toggle_list("quickfix") end
          end
        end,
      })
    end
...
local function setup_keymaps(client, bufnr)
  ...
  nnoremap("gn", require("meg.mods.lsp.rename").rename, desc("lsp: rename"))
...
local function setup_diagnostics()
  ...
  sign({ hl = "DiagnosticSignError", icon = meg.icons.lsp.error })
  sign({ hl = "DiagnosticSignWarn", icon = meg.icons.lsp.warn })
  sign({ hl = "DiagnosticSignInfo", icon = meg.icons.lsp.info })
  sign({ hl = "DiagnosticSignHint", icon = meg.icons.lsp.hint })
  ...
  diagnostic.config({
    ...
    virtual_text = {
      ...
      format = function(d)
        ...
        local icon = meg.icons.lsp[lvl:lower()]
    ...
    float = {
      ...
      prefix = function(diag, i, total)
        ...
        local prefix = fmt("%d. %s ", i, meg.icons.lsp[level:lower()])
...
local function setup_highlights(client, bufnr)
  ...
  if client then
    meg.augroup(fmt("LspHighlights%s", client.name), {
      event = { "ColorScheme" },
      pattern = bufnr,
      command = function()
...
function M.config()
  ...
  require("mason")
  -- require("meg.mods.lsp.diagnostics").setup()
  ...
  local function setup_semantic_tokens(client, bufnr)
    ...
    meg.augroup(fmt("LspSemanticTokens%s", client.name), {
  ...
  local function on_attach(client, bufnr)
    ...
    if type(caps.provider) == "boolean" and caps.colorProvider then
      if client.name == "tailwindcss" then
        -- require("meg.lsp.document_colors").buf_attach(bufnr, { single_column = true, col_count = 2 })
    ...
    --   local ok, navic = meg.require("nvim-navic")
    ...
    require("meg.mods.lsp.handlers").setup()
    -- if caps.signatureHelpProvider then require("meg.plugins.lsp.signature").setup(client) end
  ...
  local servers = require("meg.mods.lsp.servers")
  ...
  local function get_server_capabilities()
    ...
    local nvim_lsp_ok, cmp_nvim_lsp = meg.require("cmp_nvim_lsp")
