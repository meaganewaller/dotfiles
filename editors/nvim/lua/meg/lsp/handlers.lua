...
M.setup = function()
  if custom.lsp.show_diagnostic_signs then
  ...
  local config = {
    ...
    float = {
      focusable = false,
      style = "minimal",
      border = require("meg.custom").borders.single,
  ...
  vim.lsp.handlers["textDocument/signatureHelp"] = vim.lsp.with(vim.lsp.handlers.signature_help, {
    border = require("meg.custom").borders.dashed,
    -- width = 60,
    -- height = 30,
  })
end
...
local function lsp_keymaps(bufnr)
  ...
  vim.diagnostic.config({
    float = { border = require("meg.custom").borders.single },
  })
...
M.on_attach = function(client, bufnr)
  ...
  require("meg.lsp.utils").setup_document_symbols(client, bufnr)
  lsp_keymaps(bufnr)
