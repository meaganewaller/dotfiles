...
au( {'Filetype'}, { pattern={'help', 'startuptime', 'qf', 'lspinfo'}, command='nnoremap <buffer><silent> q :close<CR>', group=main})
au( {"BufNewFile","BufRead"}, { group=main, pattern="**/systemd/**/*.service", callback=function() vim.bo.filetype="systemd" end })
...
au({'TextYankPost'}, { callback=function() require'vim.highlight'.on_yank({timeout=60, higroup="Search"}) end, group=hi_yank})
au({'DirChanged'}, {pattern={'window','tab','tabpage','global'}, callback=function() vim.cmd("silent !zoxide add " .. vim.fn.getcwd()) end,group=main})
if true == false then
...
au({'BufNewFile','BufWritePre'}, {pattern={'*'}, command=[[if @% !~# '\(://\)' | call mkdir(expand('<afile>:p:h'), 'p') | endif]], group=utils })
