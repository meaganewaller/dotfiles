...
local function try_attach(bufnr)
  ...
  if #tagfiles == 0 then return end
  local attached = pcall(vim.api.nvim_buf_get_var, bufnr, "tags_attached")
  if attached then return end
  if config.on_attach then config.on_attach(bufnr) end
...
function M.setup(opts)
  ...
  vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function(params) try_attach(params.buf) end,
  })
end

local function tagjump(word, idx, regex)
  if regex then word = "/" .. word end
...
local function gen_entry_maker(opts)
  ...
  if opts.show_line then table.insert(display_items, { width = 30 }) end
  ...
  local make_display = function(entry)
    ...
    local cmd
    if opts.show_line then cmd = entry.cmd end
...
local function choose(word, candidates, callback)
  ...
  if ok then
    ...
    for _, tag in ipairs(candidates) do
      ...
      if length > max_tag_len then max_tag_len = length end
    end
...
local function jump_to(word, candidates, regex)
  ...
  elseif #candidates == 1 then
    ...
    choose(word, candidates, function(idx) tagjump(word, idx, regex) end)
  end
end

local function make_exact(word)
  local ret = word
  if string.find(word, "%^") ~= 1 then ret = "^" .. ret end
  if not string.find(word, "%$", #word - 1) then ret = ret .. "$" end
  return ret
end
