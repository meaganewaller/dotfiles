...
return {
  {
    ...
    init = function()
      ...
      vim.g["test#custom_strategies"] = {
        termsplit = function(cmd) meg.term.open(term_opts(cmd)) end,
        termvsplit = function(cmd) meg.term.open(term_opts(cmd, { direction = "vertical" })) end,
        termfloat = function(cmd) meg.term.open(term_opts(cmd, { direction = "float", focus_on_open = true })) end,
        termtab = function(cmd) meg.term.open(term_opts(cmd, { direction = "tab", focus_on_open = true })) end,
      }
      ...
      meg.nnoremap("<localleader>tn", "<cmd>TestNearest<cr>", "run _test under cursor")
      meg.nnoremap("<localleader>ta", "<cmd>TestFile<cr>", "run _all tests in file")
      meg.nnoremap("<localleader>tf", "<cmd>TestFile<cr>", "run _all tests in file")
      meg.nnoremap("<localleader>tl", "<cmd>TestLast<cr>", "run _last test")
      meg.nnoremap("<localleader>tt", "<cmd>TestLast<cr>", "run _last test")
      meg.nnoremap("<localleader>tv", "<cmd>TestVisit<cr>", "run test file _visit")
      meg.nnoremap("<localleader>tp", "<cmd>A<cr>", "open alt (edit)")
      meg.nnoremap("<localleader>tP", "<cmd>AV<cr>", "open alt (vsplit)")
    end,
  },
  {
    ...
    config = function()
      ...
      require("neotest").setup({
        ...
        floating = { border = meg.get_border() },
        icons = {
          expanded = "",
          child_prefix = "",
          child_indent = "",
