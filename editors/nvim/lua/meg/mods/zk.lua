...
return {
  ...
  config = function()
    ...
    if vim.g.picker == "fzf" then
      ...
      -- end
      --
      meg.zk_live_grep = function(opts)
        opts = opts or {}
      ...
      meg.zk_find_notes = function(opts)
        local delimiter = "\x01"
        local notes = {}
      ...
      commands.add("ZkLiveGrep", meg.zk_live_grep)
      commands.add("ZkFindNotes", meg.zk_find_notes)
      meg.nnoremap("<leader>zf", "<cmd>ZkNotes<cr>", desc("zk: find notes"))
      meg.nnoremap(
        "<leader>z/",
        function() meg.zk_live_grep({ exec_empty_query = false, cwd = vim.env.ZK_NOTEBOOK_DIR }) end,
        desc("zk: live grep")
      )
      meg.nnoremap(
        "<leader>zg",
    ...
    -- meg.nnoremap("<leader>zt", "<cmd>ZkTags<cr>", desc("zk: find tags"))
    meg.nnoremap("<leader>zn", "<Cmd>ZkNew { title = vim.fn.input('title: ') }<CR>", desc("zk: new note"))
    meg.nnoremap("<leader>nn", "<Cmd>ZkNew { title = vim.fn.input('title: ') }<CR>", desc("zk: new note"))
    meg.vnoremap("<leader>zn", function()
    ...
    meg.vnoremap("<leader>nn", function()
      vim.ui.select({ "Title", "Content" }, { prompt = "Set selection as:" }, function(choice)
        if choice == "Title" then
          vim.cmd([['<,'>ZkNewFromTitleSelection]])
        elseif choice == "Content" then
    ...
    meg.vnoremap("<leader>zg", ":'<,'>ZkMatch<CR>", desc("zk: search notes matching selection"))
    meg.nnoremap("<leader>zr", "<Cmd>ZkRecents<CR>", desc("zk: find recent notes"))
    -- meg.nnoremap(
    --   "<leader>z/",
    ...
    meg.nnoremap("gl", "<Cmd>ZkInsertLink<CR>", desc("zk: insert link"))
    meg.vnoremap("gl", ":'<,'>ZkInsertLinkAtSelection<CR>", desc("zk: insert link (selected)"))
    meg.vnoremap("gL", ":'<,'>ZkInsertLinkAtSelection {match = true}<CR>", desc("zk: insert link (search selected)"))
  end,
}
