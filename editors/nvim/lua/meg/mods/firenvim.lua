...
function M.init()
  ...
  vim.g.firenvim_config = {
    ...
    localSettings = {
      ...
      ["https?://github.com/users/meaganewaller/projects"] = {
        takeover = "never",
        priority = 1,
      },
  ...
  local function on_bufenter(evt)
    ...
    if buf_name ~= "" and _G.meg.tlen(buf_lines) <= 2 and buf_lines[1] == "" then
      vim.cmd([[startinsert]])
    else
  ...
  local function on_uienter(evt)
    ...
    local ok_headlines, headlines = meg.require("headlines")
  ...
  require("meg.globals").augroup("Firenvim", {
    {
      event = { "UIEnter" },
      once = true,
