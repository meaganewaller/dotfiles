local M = {
  ...
  config = function()
    ...
    nls.setup({
      ...
      sources = {
        ...
        format.prettierd.with({
          ...
          condition = function() return meg.executable("prettierd") end,
        }),
        format.fixjson.with({ filetypes = { "jsonc", "json" } }),
        -- format.cbfmt:with({
        --   condition = function() return meg.executable("cbfmt") end,
        -- }),
        format.stylua.with({
          condition = function()
            return meg.executable("stylua")
        ...
        --   extra_args = { "-i", "2" },
        --   condition = function() return meg.executable("beautysh") end,
        -- }),
        format.shellharden,
        format.elm_format,
        format.jq,
    ...
    local toggle_null_formatters = function()
      ...
      meg.lsp.null_formatters_enabled = not nls.enabled
      nls.toggle({ methods = nls.methods.FORMATTING })
    end

    meg.command("ToggleNullFormatters", toggle_null_formatters)
  end,
  -- },
  -- {
