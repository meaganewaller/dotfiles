...
vim.o.switchbuf = "usetab,useopen,uselast"
vim.opt.fillchars = {
  vert = "▕", -- alternatives │
  horiz = "━",
  --   horizup   = '┻',
  ...
  diff = "╱", -- alts: = ⣿ ░ ─
  msgsep = " ", -- alts: ‾ ─
  foldopen = meg.icons.misc.fold_open, -- alts: ▾
  -- foldsep = "│",
  foldsep = " ",
  foldclose = meg.icons.misc.fold_close, -- alts: ▸
}
-----------------------------------------------------------------------------//
-- Diff {{{1
-----------------------------------------------------------------------------//
...
-- vim.opt.foldtext = "v:lua.meg.folds()"
...
if meg.executable("rg") then
  vim.o.grepprg = [[rg --glob "!.git" --no-heading --vimgrep --follow $*]]
  vim.opt.grepformat = vim.opt.grepformat ^ { "%f:%l:%c:%m" }
elseif meg.executable("ag") then
...
-----------------------------------------------------------------------------//
vim.o.wildcharm = fn.char2nr(meg.replace_termcodes([[<Tab>]]))
...
vim.opt.viewoptions = { "cursor", "folds", "curdir", "slash", "unix" } -- save/restore just these (with `:{mk,load}view`)
...
-- Title {{{1
-----------------------------------------------------------------------------//
function meg.modified_icon()
  return vim.bo.modified and meg.icons.misc.circle or ""
end
vim.o.titlestring =
  "%{substitute($VIM, '.*[/\\]', '', '')} %{fnamemodify(getcwd(), \":t\")}%( %{v:lua.meg.modified_icon()}%)"
