...
config.event_handlers = {
	{
		...
		handler = function(args) -- `h: neo-tree-window-event-args`
			if require("meg.plugins.windows").auto_maximize and vim.fn.win_gettype(args.winid) ~= "popup" then
	...
	},
	{
		...
		handler = function()
			if require("meg.plugins.windows").auto_maximize then
		...
		callback = function()
			if vim.api.nvim_win_is_valid(neo_tree_win.id) then
				...
				barbar_api.set_offset(
					...
					"󰙅 " .. require("meg.utils").truc_path(vim.fn.getcwd())
				)
			end
		end,
	},
...
local function set_hls()
	if vim.g.colors_name == "rose-pine" then
		...
		nx.hl({
			{ "NeoTreeRootName", fg = colors.iris, italic = true },
			{ "NeoTreeFileName", fg = colors.text },
			{ "NeoTreeTabActive", fg = colors.text },
			{ "NeoTreeTabInactive", fg = "NeoTreeDimText:fg", bg = colors.nc },
			{ "NeoTreeTabSeparatorInactive", bg = colors.highlight_low, fg = colors.highlight_med },
