local data_dir = meg.dirs.data
local lazy_path = data_dir .. "lazy/lazy.nvim"
local modules_dir = vim.g.nvim_path .. "/lua/meg/modules"

local M = {}

function M:load_plugins()
  self.modules = {}

  local append_nativertp = function()
    package.path = package.path
		  .. string.format(";%s;%s", modules_dir .. "/configs/?.lua", modules_dir .. "/configs/?/init.lua")
  end

  local get_plugins_list = function()
		local list = {}
		local plugins_list = vim.split(vim.fn.glob(modules_dir .. "/plugins/*.lua"), "\n")
		if type(plugins_list) == "table" then
			for _, f in ipairs(plugins_list) do
				-- fill list with `plugins/*.lua`'s path used for later `require` like this:
				-- list[#list + 1] = "plugins/completion.lua"
				list[#list + 1] = f:sub(#modules_dir - 10, -1)
			end
		end
    return list
	end

	append_nativertp()

	local plugins_file = get_plugins_list()
	for _, m in ipairs(plugins_file) do
		-- require modules which returned in previous operation like this:
		-- local modules = require("meg/modules/plugins/completion.lua")
		local modules = require(m:sub(0, #m - 4))
		if type(modules) == "table" then
			for name, conf in pairs(modules) do
				self.modules[#self.modules + 1] = vim.tbl_extend("force", { name }, conf)
			end
		end
	end
end

function M:load_lazy()
  if not vim.loop.fs_stat(lazy_path) then
    local lazy_repo = "git@github.com:folke/lazy.nvim.git "
		vim.api.nvim_command("!git clone --filter=blob:none --branch=stable " .. lazy_repo .. lazy_path)
  end
  self:load_plugins()

  local clone_prefix = "git@github.com:%s.git"
  local lazy_settings = {
    	root = data_dir .. "lazy", -- directory where plugins will be installed
		git = {
			-- log = { "-10" }, -- show the last 10 commits
			timeout = 300,
			url_format = clone_prefix,
		},
		install = {
			-- install missing plugins on startup. This doesn't increase startup time.
			missing = true,
			colorscheme = { "catppuccin" },
		},
		ui = {
			-- a number <1 is a percentage., >1 is a fixed size
			size = { width = 0.88, height = 0.8 },
			wrap = true, -- wrap the lines in the ui
			-- The border to use for the UI window. Accepts same border values as |nvim_open_win()|.
			border = "rounded",
			icons = {
				cmd = meg.icons.misc.code,
				config = meg.icons.misc.gear,
				event = meg.icons.lsp.kind.Event,
				ft = meg.icons.documents.file,
				init = meg.icons.prompt,
				import = meg.icons.documents.Import,
				keys = "",
				loaded = meg.icons.misc.check,
				not_loaded = meg.icons.misc.question,
				plugin = meg.icons.misc.package,
				runtime = "",
				source = meg.icons.codicons.Method,
				start = "",
				list = {
          "",
          "",
          meg.icons.misc.chevron_right,
				},
			},
		},
		performance = {
			cache = {
				enabled = true,
				path = vim.fn.stdpath("cache") .. "/lazy/cache",
				-- Once one of the following events triggers, caching will be disabled.
				-- To cache all modules, set this to `{}`, but that is not recommended.
				disable_events = { "UIEnter", "BufReadPre" },
				ttl = 3600 * 24 * 2, -- keep unused modules for up to 2 days
			},
			reset_packpath = true, -- reset the package path to improve startup time
			rtp = {
				reset = true, -- reset the runtime path to $VIMRUNTIME and the config directory
				---@type string[]
				paths = {}, -- add any custom paths here that you want to include in the rtp
			},
		},
	}
	if vim.g.is_macos then
		lazy_settings.concurrency = 20
	end

	vim.opt.rtp:prepend(lazy_path)
	require("lazy").setup(self.modules, lazy_settings)
end

M:load_lazy()

return M
