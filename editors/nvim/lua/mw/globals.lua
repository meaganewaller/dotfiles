...
local function is_list_open(list_type)
  return mw.find(function(win) return not mw.falsy(win[list_type]) end, fn.getwininfo()) ~= nil
end
...
function mw.nightly() return vim.version().minor >= LATEST_NIGHTLY_MINOR end
    ...
    return {
      ...
      command = function(args)
        mw.foreach(function(value, scope)
          ...
          if scope == "plugins" then return mw.ftplugin_conf(value) end
          if type(value) ~= "table" and vim.is_callable(value) then return value(args) end
          mw.foreach(function(setting, option) vim[scope][option] = setting end, value)
        end, settings)
      end,
    }
  end, map)
...
---@param str string
---@return string
function mw.replace_termcodes(str) return api.nvim_replace_termcodes(str, true, true, true) end

---check if a certain feature/version/commit exists in nvim
---@param feature string
---@return boolean
function mw.has(feature) return fn.has(feature) > 0 end
