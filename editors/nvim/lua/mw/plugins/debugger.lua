local fn = vim.fn
local icons, highlight, border, palette = mw.ui.icons, mw.highlight, mw.ui.current.border, mw.ui.palette

mw.debug = { layout = { ft = { dart = 2 } } }
...
return {
  {
    ...
    keys = {
      ...
      {
        '<localleader>duc',
        function() require('dapui').close(mw.debug.layout.ft[vim.bo.ft]) end,
        desc = 'dap ui: close',
      },
      {
        '<localleader>dut',
        function() require('dapui').toggle(mw.debug.layout.ft[vim.bo.ft]) end,
        desc = 'dap ui: toggle',
      },
    ...
    config = function()
      ...
      dap.listeners.after.event_initialized['dapui_config'] = function() dapui.open(mw.debug.layout.ft[vim.bo.ft]) end
    end,
    dependencies = {
      {
        {
