...
local bgcolor = function()
  if BrioVim.colorscheme == 'nightfly' then
    return '#011627'
  else
    return nil;
  end
...
table.insert(gls.left, {
  DiagnosticError = {
    ...
    icon = BrioVim.icons.errorOutline,
    separator_highlight = { colors.gitbg, colors.bg },
...
table.insert(gls.left, {
  DiagnosticHint = {
    ...
    icon = BrioVim.icons.lightbulbOutline,
...
table.insert(gls.left, {
  DiagnosticInfo = {
    ...
    icon = BrioVim.icons.infoOutline,
...
if BrioVim.statusline.path_enabled then
  table.insert(gls.right, {
    FileName = {
      provider = function()
        ...
        if BrioVim.statusline.path == 'relative' then
