...
| [0004](0004-claude-settings-management.md) | Claude settings management | 2026-05-16 | proposed | Hybrid: static lists (permissions, hook metadata, plugins, feature flags) move to `home/.chezmoidata/claude-*.yaml` rendered by `home/dot_claude/settings.json.tmpl`; `bin/sync-claude-settings` shrinks to a Bedrock-only runtime overlay; `settings.local.json` stays unmanaged. |
| [0005](0005-universal-theme-switcher.md) | Universal `theme` switcher | 2026-05-20 | proposed | `theme <name>` on `$PATH` writes a state file at `~/.local/state/theme/current` and runs every applier under `home/dot_local/libexec/dotfiles/theme.d/`; catalog of theme → per-tool palette mapping lives in `home/.chezmoidata/themes.yaml`. Switches never dirty the chezmoi source tree. |
