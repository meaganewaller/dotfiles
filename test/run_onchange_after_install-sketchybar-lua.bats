...
@test "rendered script has a CI skip guard" {
	...
	[[ "$output" == *"Running in CI, skipping SbarLua build"* ]] || fail "output was: $output"
}
...
@test "skips cleanly when the external source hasn't been cloned yet" {
	...
	[ "$status" -eq 0 ] || fail "status=$status output=$output"
	[[ "$output" == *"chezmoi external not applied yet"* ]] || fail "output was: $output"
}
...
@test "skips cleanly when make is unavailable" {
	...
	[ "$status" -eq 0 ] || fail "status=$status output=$output"
	[[ "$output" == *"make not found"* ]] || fail "output was: $output"
}
...
EOF
	...
	[ "$status" -eq 0 ] || fail "status=$status output=$output"
	[[ "$output" == *"SbarLua installed: $module"* ]] || fail "output was: $output"
}
...
EOF
	...
	[[ "$output" == *"check the build output above"* ]] || fail "output was: $output"
}
