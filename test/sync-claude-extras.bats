...
@test "--check reports ok / add / drift without mutating anything" {
	...
	[[ "$output" == *"ok: mp-existing"* ]] || fail "output was: $output"
	[[ "$output" == *"add mp-new"* ]] || fail "output was: $output"
	[[ "$output" == *"ok: p-existing@mp-existing"* ]] || fail "output was: $output"
	[[ "$output" == *"install p-new@mp-existing"* ]] || fail "output was: $output"

	# An installed-but-undeclared plugin is reported as drift, not removed.
	[[ "$output" == *"drift: plugin p-orphan@mp-existing"* ]] || fail "output was: $output"

	# A built-in marketplace is never flagged as drift (not ours to manage)...
	[[ "$output" != *"drift: marketplace claude-plugins-official"* ]] || fail "output was: $output"
	# ...but a non-built-in undeclared marketplace still is.
	[[ "$output" == *"drift: marketplace mp-orphan"* ]] || fail "output was: $output"
...
@test "skips cleanly when the claude CLI is absent" {
	...
	[ "$status" -eq 0 ] || fail "status=$status output=$output"
	[[ "$output" == *"claude not found"* ]] || fail "output was: $output"
}
