...
@test "fails when the source applier directory is missing" {
  ...
  run env -u XDG_STATE_HOME HOME="$TEST_HOME_DIR" bash "$rendered"
  [ "$status" -eq 1 ]
  [[ "$output" == *"source applier dir missing"* ]]
}
...
@test "fails when the default palette references a tool with no applier" {
  ...
  run env -u XDG_STATE_HOME HOME="$TEST_HOME_DIR" bash "$rendered"
  [ "$status" -eq 1 ]
  [[ "$output" == *"references tools with no applier"* ]]
  [[ "$output" == *"foobar"* ]]
}
...
@test "fails when an applier has no matching key in the default palette" {
  ...
  run env -u XDG_STATE_HOME HOME="$TEST_HOME_DIR" bash "$rendered"
  [ "$status" -eq 1 ]
  [[ "$output" == *"missing from default palette"* ]]
  [[ "$output" == *"extra-tool"* ]]
}
...
EOF
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"applying theme 'catppuccin-mocha'"* ]]
  [ "$(cat "$theme_log")" = "catppuccin-mocha" ]
}
...
EOF
  ...
  run env -u XDG_STATE_HOME HOME="$stub" bash "$rendered"
  [ "$status" -eq 0 ]
  [[ "$output" == *"applying theme 'catppuccin-latte'"* ]]
  [ "$(cat "$theme_log")" = "catppuccin-latte" ]
}
