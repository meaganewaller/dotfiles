function fin --description "ggl wrapper for frontend developers"
    ...
    if set -q _flag_version
        functions --query ggl; and eval ggl -v
        ...
        for i in (seq (count $list_cmd))
            ...
            if set -q _flag_long || test "$argv[2]" = long
                ...
                test "$flag_query" = true; and echo " " $stamp_query $ca"query: "$co$tb (string join "" "https://" $$item_query) $cn
                test "$flag_docs" = true; and echo " " $stamp_docs $ca"docs: "$co$tb (string join "" "https://" $$item_docs) $cn
                test "$flag_docs_query" = true; and echo " " $stamp_docs_query $ca"docs query: "$co$tb (string join "" "https://" $$item_docs_query) $cn
            end
        end
        return
    end
