...
#     fi
#     case "${type}" in
#       "'Issue'") COLOR=$FOAM; ICON=$GIT_ISSUE; URL="$(gh api "$(echo "${url}" | sed -e "s/^'//" -e "s/'$//")" | jq .html_url)"
#       ;;
#       "'Discussion'") COLOR=$TEXT; ICON=$GIT_DISCUSSION; URL="https://www.github.com/notifications"
#       ;;
#       "'PullRequest'") COLOR=$IRIS; ICON=$GIT_PULL_REQUEST; URL="$(gh api "$(echo "${url}" | sed -e "s/^'//" -e "s/'$//")" | jq .html_url)"
#       ;;
#       "'Commit'") COLOR=$TEXT; ICON=$GIT_COMMIT; URL="$(gh api "$(echo "${url}" | sed -e "s/^'//" -e "s/'$//")" | jq .html_url)"
#       ;;
#     esac
#
#     if [ "$IMPORTANT" != "" ]; then
