...
```bash
gh api repos/GITHUB_USERNAME/GITHUB_REPO/pulls/PR_NUMBER/comments | jq '.[] | {id, path, line, body: .body[0:100], author: .user.login}'
```
...
```bash
gh api -X POST repos/GITHUB_USERNAME/GITHUB_REPO/pulls/PR_NUMBER/comments/COMMENT_ID/replies \
...
query {
  repository(owner: "GITHUB_USERNAME", name: "GTHUB_REPO") {
...
See: https://github.com/GITHUB_USERNAME/GITHUB_REPO/commit/COMMIT_ID#diff-ID
```
